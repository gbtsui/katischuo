...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	const searchForAll = parseIfAllExercises(url);
	...
	if (!searchForAll) {
		...
		const sets = (
			await db
				...
				.where(and(inArray(set.exerciseId, ids), eq(workout.userId, locals.session.userId)))
		).map((astaghfirullah) => astaghfirullah.set);
		const oneRepMaxes = sets.reduce(
			(acc, s) => {
				const epley1RM = Calculate1RMEpley(
					parseFloat(s.weight as string),
					parseFloat(s.reps as string)
				);
				const brzycki1RM = Calculate1RMBrzycki(
					parseFloat(s.weight as string),
					parseFloat(s.reps as string)
				);
				...
				if (
					!acc[s.exerciseId] ||
					average1RM > (acc[s.exerciseId].epley + acc[s.exerciseId].brzycki) / 2
				) {
		...
		const sets = (await db
			...
			.from(set)
			.innerJoin(workout, eq(set.workoutId, workout.id))
			.where(eq(workout.userId, locals.session.userId))).map((sayWallahiBro) => sayWallahiBro.set)
		;
		console.log(sets)
...
function parseIfAllExercises(url: URL) {
