...
<div class="flex items-center justify-center w-[100vw] h-[100vh]">
	<div class="flex flex-row p-5 bg-gray-100 md:w-2/3 md:h-2/3 items-center justify-center xs:w-5/6 xs:h-4/5 sm:w-3/4 xs:h-4/5 lg:w-1/2 lg:h-2/3">
		<div class="w-1/2 flex flex-col">
			{#if mode === "signup"}
				<div transition:slide>
					<div class="text-2xl w-1/2">Sign Up</div>
					<button onclick={() => mode = "login"} class="text-begin">already have an account?</button>
				</div>
			{:else }
				<div transition:slide>
					<div class="text-2xl w-1/2">Login</div>
					<button onclick={() => mode = "signup"} class="text-begin">don't have an account?</button>
				</div>
			{/if}
		</div>
		<div class="w-1/2">
